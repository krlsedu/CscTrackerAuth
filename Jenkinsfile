...
pipeline {
    ...
    stages {
                ...
                script {
                    ...
                    withCredentials([usernamePassword(credentialsId: 'gitHub', passwordVariable: 'password', usernameVariable: 'user')]) {
                        script {
                            ...
                            sh 'git pull https://krlsedu:${password}@github.com/krlsedu/CscTrackerAuth.git HEAD:' + env.BRANCH_NAME
                            ...
                            sh 'git push https://krlsedu:${password}@github.com/krlsedu/CscTrackerAuth.git HEAD:' + env.BRANCH_NAME
                        }
                    }
        ...
        stage('Docker image') {
            ...
            steps {
                sh 'docker build -t krlsedu/csctracker-auth:latest -t krlsedu/csctracker-auth:' + env.VERSION_NAME + ' .'
            }
        }
        stage('Docker Push') {
            agent any
            ...
            steps {
                withCredentials([usernamePassword(credentialsId: 'dockerHub', passwordVariable: 'dockerHubPassword', usernameVariable: 'dockerHubUser')]) {
                    ...
                    sh 'docker push krlsedu/csctracker-auth'
                    sh 'docker push krlsedu/csctracker-auth:' + env.VERSION_NAME
                }
            }
        }
        ...
        stage('Service update') {
            ...
            steps {
                sh 'docker service update --image krlsedu/csctracker-auth:' + env.VERSION_NAME + ' csctracker_services_auth'
            }
        }
    }
}
...
pipeline {
  ...
  stages {
    ...
    stage('Docker image') {
      agent any
      steps {
        sh 'docker build -t krlsedu/csctracker-auth:latest .'
      }
    }
    stage('Docker Push') {
      agent any
